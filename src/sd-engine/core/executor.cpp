...
static std::map<std::string, int> compute_node_depths(
...
sd_error_t DAGExecutor::execute(Workflow* workflow, const ExecutionConfig& config) {
    ...
    int max_threads = config.max_threads > 0 ? config.max_threads :
    ...
    for (int depth = 0; depth <= max_depth; depth++) {
        ...
        if (config.verbose) {
            printf("[Executor] Layer %d: %zu nodes (max_threads=%d)\n",
                   depth, layer_nodes.size(), max_threads);
        }
        ...
        for (const auto& node_id : layer_nodes) {
            futures.push_back(std::async(std::launch::async, [this, workflow, &config,
                ...
                
                {
                    ...
                    if (config.verbose) {
                        printf("[Executor] Executing node %s (%s) [%d/%d] (thread=%zu)\n", 
                               node_id.c_str(), node->get_class_type().c_str(),
                               current, total,
...
sd_error_t DAGExecutor::prepare_inputs(Workflow* workflow,
                                       const std::string& node_id,
                                       NodeInputs& inputs,
                                       std::map<std::string, NodeOutputs>& computed) {
    Node* node = workflow->get_node(node_id);
    if (!node) return sd_error_t::ERROR_INVALID_INPUT;
...
sd_error_t DAGExecutor::execute_node(Node* node,
