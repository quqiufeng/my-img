...
std::string WorkflowBuilder::add_lora_loader(const std::string& lora_path,
...
std::string WorkflowBuilder::add_upscale_model_loader(const std::string& model_path,
                                                      bool use_gpu,
                                                      int tile_size) {
    std::map<std::string, json> inputs;
...
std::string WorkflowBuilder::add_canny_preprocessor(const std::string& image_node_id,
                                                    int low_threshold,
                                                    int high_threshold) {
    std::map<std::string, json> inputs;
    inputs["image"] = make_link(image_node_id, 0);
...
std::string WorkflowBuilder::add_ipadapter_loader(const std::string& model_path,
...
std::string WorkflowBuilder::add_clip_set_last_layer(const std::string& clip_node_id,
                                                     int stop_at_clip_layer) {
    std::map<std::string, json> inputs;
...
std::string WorkflowBuilder::add_clip_vision_encode(const std::string& clip_node_id,
                                                    const std::string& image_node_id) {
    std::map<std::string, json> inputs;
    inputs["clip"] = make_link(clip_node_id, 0);
    inputs["image"] = make_link(image_node_id, 0);
...
std::string WorkflowBuilder::add_image_color_adjust(const std::string& image_node_id,
                                                    float brightness,
                                                    float contrast,
                                                    float saturation) {
    std::map<std::string, json> inputs;
...
std::string WorkflowBuilder::add_ksampler_advanced(const std::string& model_node_id,
                                                   const std::string& positive_node_id,
                                                   const std::string& negative_node_id,
                                                   const std::string& latent_node_id,
                                                   int seed,
...
// ============================================================================
std::string IPAdapterTxt2ImgBuilder::build(const std::string& ckpt_path,
                                           const std::string& prompt,
                                           const std::string& negative_prompt,
    ...
    std::string ipadapter = builder.add_ipadapter_loader(
        ipadapter_path, cross_attention_dim, num_tokens, clip_embeddings_dim);
    std::string ref_image = builder.add_load_image(reference_image);
    std::string ipadapter_positive = builder.add_ipadapter_apply(
        positive, ipadapter, ref_image, ipadapter_strength);
    
    std::string sampler = builder.add_ksampler(
        loader, ipadapter_positive, negative, latent, seed, steps, cfg, "euler", 1.0f);
    std::string decoded = builder.add_vae_decode(sampler, loader);
    builder.add_save_image(decoded, output_prefix);
