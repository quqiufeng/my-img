...
#### `img_to_video_v1.sh` - 图片+SFT预设音色生成短视频

**功能**：将一组图片配上中文SFT预设女声、字幕，自动生成带配音的短视频。

**参数**：
```bash
$1 图片     : 图片文件夹/单张图片/空格分隔的图片路径
$2 每张秒数 : 每张图片展示时长(秒)，默认2秒。实际时长会根据配音自动调整
$3 文案     : 用 | 分隔每段文字，如 "第一句|第二句|第三句"
$4 输出视频 : 输出文件路径，默认 output.mp4
$5 延迟     : 音频延迟，默认0.6秒（预留参数）
```
...
**特点**：
- 使用 CosyVoice-300M-SFT 预设音色（中文女声），无需参考音频
...
#### `img_to_video_v2.sh` - 图片+声音克隆生成短视频

**功能**：与 v1 相同，但支持声音克隆，可使用自定义参考音频替代预设音色。
...
$3 文案     : 用 | 分隔每段文字，如 "第一句|第二句|第三句"
$4 参考音频 : 用于克隆声音的音频文件路径 (3-30秒 wav 格式)
