...
class Workflow {
  public:
    ...
    /// @brief 设置最大允许节点数（防止恶意构造超大 DAG）
    static void set_max_nodes(size_t max_nodes);

    /// @brief 获取当前最大允许节点数
    static size_t get_max_nodes();
  ...
  private:
    ...
    /// @brief 解析原生简化 JSON 格式
    bool parse_native_json(const std::string& json_str);
};
