...
### 14.3 待实现功能（图像质量、风格、压缩、清晰度相关）

以下功能需要自行实现或在 libtorch 层扩展，**sd.cpp 当前不支持**：

#### 图像质量提升（生成阶段）

| 功能 | 描述 | 优先级 | 实现方案 |
|------|------|--------|----------|
| **FreeU** | UNet 跳跃连接频域滤波，提升图像细节和质量 | P1 | 修改 UNet 前向传播，注入 `b1/b2/s1/s2` 参数 |
| **SAG** | Self-Attention Guidance，采样过程注意力引导 | P2 | 修改采样器，对模糊 latent 计算 attention 差异 |
...
| 功能 | 描述 | 优先级 | 实现方案 |
|------|------|--------|----------|
| **智能锐化** | 边缘感知锐化，避免过度锐化噪声 | P1 | Sobel/Canny 边缘检测 + 边缘 mask |
| **边缘蒙版锐化** | 仅锐化边缘区域，保护平滑区域 | P1 | 高反差保留 + 边缘权重 |
| **高通滤波锐化** | 频域锐化，增强细节 | P2 | FFT 高通滤波 |
| **超分辨率** | Real-ESRGAN / SwinIR / BSRGAN | P1 | 接入专用超分模型 |
| **AI 放大** | 基于深度学习的图像放大（Real-ESRGAN+） | P1 | 加载 .pth 模型进行推理 |

#### 风格与色彩（后处理）

| 功能 | 描述 | 优先级 | 实现方案 |
|------|------|--------|----------|
| **LUT/CUBE 预设** | 加载专业 LUT 文件（.cube, .3dl） | P1 | 解析 CUBE 格式，3D LUT 查找 |
| **色调分离** | 高光/阴影分别着色 | P2 | 分区颜色叠加 |
| **胶片模拟** | 颗粒、色偏、宽容度模拟 | P2 | 噪声 + 色彩曲线 |
| **色彩查找表** | 3D LUT 颜色分级 | P2 | 33x33x33 查找表 |
| **去雾/除霾** | 大气散射模型去雾 | P2 | 暗通道先验或深度学习 |
| **HDR 效果** | 高动态范围 tone mapping | P2 | Reinhard/ACES tone mapping |
| **夜景增强** | 低光照图像增强 | P3 | 亮度提升 + 降噪 |

#### 压缩性能与格式

| 功能 | 描述 | 优先级 | 实现方案 |
|------|------|--------|----------|
| **JPG 输出** | JPEG 格式输出（带质量参数） | P1 | libjpeg / libjpeg-turbo |
| **WEBP 输出** | WebP 格式（更好的压缩率） | P1 | libwebp |
| **AVIF 输出** | AV1 图像格式（最佳压缩） | P2 | libavif |
| **PNG 压缩级别** | 控制 PNG 压缩比 | P2 | libpng 压缩参数 |
| **批量压缩** | 统一压缩目录下所有图片 | P2 | 批量处理 + 格式转换 |

#### 图像质量修复（后处理）

| 功能 | 描述 | 优先级 | 实现方案 |
|------|------|--------|----------|
| **高级降噪** | 非局部均值（NLM）、BM3D | P1 | 接入 OpenCV 或自研 |
| **镜头校正** | 畸变校正、色差修复 | P2 | 镜头参数模型 |
| **透视校正** | 透视变换，拉正倾斜图像 | P2 | 单应性矩阵变换 |
| **摩尔纹去除** | 去除扫描/屏幕摩尔纹 | P3 | 频域滤波 |
| **抖动/去交错** | 修复视频帧/扫描图像 | P3 | 时域/空域滤波 |

#### 局部调整（后处理）

| 功能 | 描述 | 优先级 | 实现方案 |
|------|------|--------|----------|
| **径向滤镜** | 圆形区域调整（曝光/对比度/饱和度） | P1 | 距离 mask + 参数混合 |
| **渐变滤镜** | 线性渐变调整（模拟 GND 滤镜） | P1 | 线性 mask + 参数混合 |
| **画笔调整** | 自由绘制调整区域 | P3 | 需要 UI 支持 |

**FreeU 详细说明**：
- **原理**：在 UNet 解码器的跳跃连接中，对 backbone 特征进行缩放（`b1/b2`），对 skip 特征进行频域滤波（`s1/s2`）
- **效果**：显著提升图像细节和真实感，无需额外训练
- **难点**：需要修改 sd.cpp 的 UNet 实现或在其之上包装
- **参考**：FreeU: Free Lunch in Diffusion U-Net (https://github.com/ChenyangSi/FreeU)
